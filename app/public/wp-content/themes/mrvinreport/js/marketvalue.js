var VHRreports = VHRreports || {};
VHRreports.dom = VHRreports.dom || new function() {
  this.loadedFiles = {};
  ...
  this.fadeIn = function(id, startDelay, fadeDelay, displayStyle) {
    ...
    if (startDelay > 0) {
      setTimeout(function() {
        VHRreports.dom.fadeIn(id, 0, fadeDelay, displayStyle);
      }, startDelay);
    } else {
...
VHRreports.utils = VHRreports.utils || new function() {
  var VA = VHRreports;
...
VHRreports.MarketValueWidget = new function() {
	var VA = VHRreports;
  var key = 'VA_DEMO_KEY';
  var vin = null;
  var year = null;
  ...
  this.render = function(options) {
    if (!window.Chart || !window.Chart.types || !window.Chart.types.Line) {  // "Chart" not loaded yet? Retry...
      setTimeout(function() {
        VHRreports.MarketValueWidget.render(options);
      }, 250);
      return;
    }
    /* if (options.vin) { */
  ...
  var reloadWidget = function(newVin) {
    ...
    var dataUrl = '';
    if (vin != '') {
      dataUrl = '//marketvalue.vinaudit.com/getmarketvalue.php?key='+encodeURIComponent(key)+'&vin='+encodeURIComponent(vin)+'&period='+encodeURIComponent(period)+'&mileage='+encodeURIComponent(mileage);
    } else {
      dataUrl = '//marketvalue.vinaudit.com/getmarketvalue.php?key='+encodeURIComponent(key)+'&year='+encodeURIComponent(year)+'&make='+encodeURIComponent(make)+'&model='+encodeURIComponent(model)+'&trim='+encodeURIComponent(trim)+'&period='+encodeURIComponent(period)+'&mileage='+encodeURIComponent(mileage);
    }*/
    hideGraphElements(newVin);
    onJsonReady(got_json);
  ...
  var hideGraphElements = function(newVin) {
    ...
    VHRreports.MarketValueWidget.destroy();
  };
  ...
  var onJsonReady = function(json) {
    ...
    var setTooltipConfiguration = function(activePoint) {
      if (activePoint) {
        ...
        var showPrice = activePoint.label.replace('$', '').replace(',', '');
        if (mean > 0 && stdev > 0) {
  ...
  var initMileageSelect = function(currentMileage, price) {
	VA.dom.fill('va_mv_mileage_text', (currentMileage ? VA.utils.formatNumber(currentMileage) + ' miles' : 'Unknown'));
    ...
    for (var i = 0; i <= maxI;) {
      var value = i*1000;
      VA.dom.selectAddOption(mileageSelect, value, VA.utils.formatNumber(value) + ' miles');
      i += 25;
      if (currentMileage > value && (currentMileage < i*1000 || i > maxI)) {
        VA.dom.selectAddOption(mileageSelect, currentMileage, VA.utils.formatNumber(currentMileage) + ' miles');
      }
    }
    ...
    mileageSelect.onchange = function() {
      if (mileageSelect.style.display == '') {
        ...
        VHRreports.MarketValueWidget.reRender({
          'mileage': mileage,
          'price' : price
        });
...
VHRreports.dom.loadCss('//fonts.googleapis.com/css?family=Open+Sans');
VHRreports.dom.loadCss(urlcommon+'mv/styles.css');
VHRreports.dom.loadJs(urlcommon+'mv/chart.js');
*/
